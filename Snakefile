...
rule samtools_sort:
	...
	shell:
		"samtools sort -@ {threads} -T tmp/alignment/{wildcards.sample}  "
...
rule bqsr:
	...
	shell:
		"/software/gatk-4.1.2.0/gatk --java-options '-Xmx50G' BaseRecalibrator -R {params.ref} --known-sites {params.snp_ref} --tmp-dir tmp/bqsr -I {input.ip} -O {output} 2>{log}"
